...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const [currentEmpRows] = await db.query('SELECT dept_id FROM employees WHERE emp_id = ?', [id]);
    ...
    const oldDeptId = currentEmpRows[0].dept_id;
    ...
    // If department changed, update counts and get names
    if (oldDeptId !== dept_id) {
      // Get old and new department names
      const [[oldDept]] = await db.query('SELECT name FROM departments WHERE dept_id = ?', [oldDeptId]);
      const [[newDept]] = await db.query('SELECT name FROM departments WHERE dept_id = ?', [dept_id]);

      oldDeptName = oldDept?.department || 'Unknown';
      newDeptName = newDept?.department || 'Unknown';
    ...
    return res.json({
      message: oldDeptId !== dept_id 
        ? `Employee transferred from ${oldDeptName} to ${newDeptName}`
