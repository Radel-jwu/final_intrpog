...
router.post('/', async (req, res) => {
  const { acc_id, title, firstname, lastname, email, password, role = 'user' , status} = req.body;
  ...
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO accounts (acc_id,  title, firstname, lastname,  email, password, role, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [acc_id, title, firstname, lastname, email,  hashedPassword, role, status]
    );
...
router.put('/:id', auth, async (req, res) => {
  const id = req.params.id;
  const { acc_id, title, firstname, lastname, email, password, role, status } = req.body;
  ...
  try {
    ...
    // Update acc_id if provided (optional, but usually primary keys shouldn't change)
    if (acc_id) {
      updateFields.push('acc_id = ?');
      params.push(acc_id);
    }
